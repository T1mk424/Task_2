...
class TestCreateUser:
    ...
    def test_create_user(self):
        ...
        with allure.step('Проверка,что пользователь успешно создался,код ответа 200 и получили accessToken'):
            assert response.status_code == 200 and 'accessToken' in response.text, (f'Ожидалось 200, получили {response.status_code}, ожидалось "accessToken", получили {response.text}')
