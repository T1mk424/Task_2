import requests
import pytest
from data.data import Data, Urls, ErrorMessage
import allure
...
class TestUpdateUser:
    ...
    def test_update_user_data_success(self, update_argument, update_data, create_user_and_get_token):
        ...
        with allure.step('Авторизация и отправка запроса на обновление данных пользователя'):
            update_user = requests.patch(f'{Urls.MAIN_URL}{Urls.API_GET_USER}', data = user_data, headers = {'Authorization': create_user_and_get_token})
