...
from data.data import Data,Urls,ErrorMessage,Burgers
...
class TestCreateOrder:
    ...
    def test_create_order_with_auth(self, create_user_and_get_token, get_ingredient_hash):
        ...
        with allure.step('Выборка ингредиентов для создания заказа'):
            ingredients = {'ingredients': [get_ingredient_hash['data'][0]['_id'], get_ingredient_hash['data'][1]['_id'], get_ingredient_hash['data'][13]['_id']]}
        ...
        with allure.step('Проверка, что имя "Экзо-плантаго бессмертный флюоресцентный бургер" и заказ создался и код ответа 200'):
            assert order['name'] == Burgers.ECSO_UNDEAD_CRATER_FLY_BURGER and order['success'] == True and response.status_code == 200

    @allure.title("Проверка создания заказа без авторизации")
    def test_create_order_without_auth(self, get_ingredient_hash):
        with allure.step('Выборка ингредиентов для создания заказа'):
            ingredients = {'ingredients': [get_ingredient_hash['data'][4]['_id'], get_ingredient_hash['data'][3]['_id'], get_ingredient_hash['data'][8]['_id']]}
        ...
        with allure.step('Проверка,что имя "Spicy краторный био-марсианский бургер" и заказ создался  и код ответа 200'):
            assert order['name'] == Burgers.SPICY_CRATER_BIO_BURGER and order['success'] == True and response.status_code == 200
    ...
    def test_create_order_with_ingredients(self, create_user_and_get_token, get_ingredient_hash):
        with allure.step('Выборка ингредиентов для создания заказа'):
            ingredients = {'ingredients':[get_ingredient_hash['data'][0]['_id'], get_ingredient_hash['data'][2]['_id'], get_ingredient_hash['data'][14]['_id']]}
        ...
        with allure.step('Проверка,что имя  "Метеоритный флюоресцентный астероидный бургер" и заказ создался и код ответа 200'):
         assert order['name'] == Burgers.ASTEROID_METEOR_FLY_BURGER and order['success'] == True and response.status_code == 200
